...
**标注为已下线的（6 条）**，脚本按`已废弃`/`已从场景基线中去除`自动跳过：
...
其中 p18、p25 的主题已由 p17、p24 的完整页覆盖，无影响。真正有缺口的是三处：**p11 物理线路错连**（独立场景，p7 的 OSPF 两端 IP 不同网段不能替代）；**p27/p28 ISIS 路由环路的具体案例**（p29 有 ping + tracert 的定位步骤，但停在"定位导致环路的配置：待定"，缺的正是这两页）；**p30 二层环路**（概览表列了这个根因，但全篇没有任何排障步骤，落不成 skill）。这三处需要补充原始素材。
