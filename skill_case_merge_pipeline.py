...
- **每篇skill都要有"诊断结论输出要求"小节**，规定agent排查完必须交付什么：定位到根因时输出「故障对象」（具体到网元名、端口/接口/会话名）、「根因类型」（列出本篇覆盖的取值）、「原因」（一句自然语言，且必须把故障对象写进句子里）、「修复建议」；未定位到根因时必须明确输出"未找到根因"并列出已执行的检查项与结论，不许用推测代替根因。
- **需要变更配置的skill还要有"修复方案输出要求"小节**：按方案逐条输出「方案序号」（方案一、方案二……）、「方案说明」、「修复对象名」（哪个网元）、「CLI序列」；修复走管控接口时给出接口路径与入参并注明不下发CLI。参数取值来自数据查询接口、资源分配接口、配置下发接口三类。有多个可选方案（如两端二选一）时全部列出交人选择，不要替用户决定。"""
