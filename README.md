...
一个 sheet 里的多个场景靠**场景列的合并单元格**分块（「排障目标」，没有这一列就用「组网场景」）：一个合并区间 = 一个场景，区间覆盖的行是它的排障步骤。这一列整列为空时（有的表那格放的是截图）退回按**步骤编号**分块，编号回到 1 就是下一个场景。场景名依次退：排障目标 → 组网场景 → sheet 名。
...
两张基准表、两份格式基准：`排障步骤表.xlsx` + `sample_skill.md` 是 11 列的完整形态，`CPU利用率超限步骤表.xlsx` + `sample_skill_cpu.md` 只有 6 列（没有排障目标/回显/修复建议/影响性/修复验证），参数是裸词或 `[ ]` 圈着的，还有一条命令只出现在「步骤详细描述」里。xlsx 是二进制、进 git 看不出 diff，所以表的内容以 `excel_cases/build_*_sheet.py` 为准，改表改脚本再重跑。
...
- `excel_skill_distill_pipeline.py` 默认处理 `excel_cases/` 下的**每一个** xlsx，其中包括仓库自带的两张样例表 `排障步骤表.xlsx`、`CPU利用率超限步骤表.xlsx`（它们同时是测试的输入，别删）。只想跑自己那张表时，把 `XLSX_DIR` 指到具体文件。
