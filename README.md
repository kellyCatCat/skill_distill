...
这份测试真起服务、真发浏览器那种 multipart 请求传 xlsx、真过校验器、真写文件，只把 `requests.post` 换掉，守住这几件事：上传阶段一次模型都不调、生成后能拿到全文、三种下载都对、落盘写出文件且重复落盘不重复写、落盘后能在 skill 库面板里看到它、违规回复判失败且一个文件都不写出去、坏输入（非 xlsx / 越界的场景号 / 不存在的任务号）落在 4xx。
