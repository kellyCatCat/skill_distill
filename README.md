...
"（情形N）"。未识别的列不丢弃，原样输出为"其它信息"。

表里带"排障目标"这类场景列时，**一张表可以含多个故障场景**：该格非空即新场景开始，
后续步骤行留空表示接着上一个场景（合并单元格的形态）。此时按 `## 场景N：…` 分节、
步骤降为 `###`，并把该列里的告警名与构造方法放在场景开头；不这么切的话，第二个场景的
步骤会和第一个场景的平铺在同一层，蒸馏时两条排查链会混成一条。转完会列出识别到的场景，
**要核对场景边界切对了没有**。另外，"修复建议"为空而"影响性"有内容时会告警——
这通常是源表那一行整列右移了，脚本严格按表头列位映射，源表错位就会照着错位输出。正文里的外部链接会被列出来
提醒——蒸馏的写作约束要求删掉链接，必要内容得先摘成文字。输入支持 `.xlsx`（需
`pip install openpyxl`）和 `.csv/.tsv`（从 Excel 直接粘贴出来的形态，无需依赖）。

**改成分支格式再蒸馏**：`restyle_docs.py` 调大模型把源文档改写成"一步一个有序列表项、
判断分支挂成子项"的形态，**同时把回显整段删掉**（skill 里本来就不留大段回显）：
...
1. **检查接口物理及协议状态**
   ...
   在任意视图下执行 `display interface <interface-type> <interface-number>` 检查入接口是否收到数据报文。
   - 若接口状态为 `Down`，请检查线路连接及接口是否被 `shutdown`。
   - 若接口状态为 `Up` 但无流量，继续后续步骤。
```

回显在**送模型之前**就删掉了：省 token 之外更重要的是，模型看不到的东西不可能被它
改写或编造。带"回显"标签的围栏块整块删除，命令块保留。改写靠模型，所以重点在**把它
能改动的范围钉死**——落盘前逐项校验，不合规就不写文件、原文保持不变：

| 校验项 | 拦的是什么 |
| --- | --- |
| 送进去的命令必须一条不少地回来 | 模型改写时把某条命令漏掉 |
| **不得出现原文没有的命令** | 模型"补全"出一条设备上不存在的命令，agent 会照着执行 |
| 每个步骤标题、场景标题都要还在 | 合并或漏掉步骤 |
| 实质字符数不得低于原文 70% | "其余步骤同上"这类概括（只数汉字与字母数字，不算排版符号） |
| 围栏必须闭合 | 输出被截断 |
...
| `restyle_docs.py` | 调模型把源文档改写成分支格式并删掉回显（加 `--apply` 才覆盖） |
