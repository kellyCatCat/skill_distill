...
**校验器会拦下的东西**（不合规的不落盘，只记进报告，并把原因回传给模型让它改）：命令没用反引号、参数不是 `<>` 形式、同一参数多种写法、残留表内编号、编造表里没有的命令、修复 CLI 里抄了回显的具体值、正文用了没申报的参数、前置检查里有跳转、根因对照表漏了步骤表写明的根因、跳转指向不存在的步骤。报错会带上**步骤表位置**（步骤号、sheet、行号）和正文里最接近的那段命令。
