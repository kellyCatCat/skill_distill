...
**markdown 是渲染着看的**：生成结果每篇有「预览（渲染后）」和「查看原文」两个折叠块，skill 库每行有「预览」和「编辑」。渲染器是页面里自己写的一小段（内网拉不到 CDN，而 skill 用到的无非是标题、列表、表格、围栏和行内代码），有三处是踩出来的：正文先整体转义再插标签，所以 `<endpoint-ipv6>` 会原样显示而不是被当成标签吞掉；表格单元格按 `|` 切之前先把转义过的 `\|` 藏起来，否则 `display paf \| include X` 会被切成两格；空行不关列表，否则 `<ol>` 会从 1 重新数、"步骤2"显示成"步骤1"。这几条在 `test_web_server.py` 里有断言（用 node 跑，没装 node 就跳过）。

**人工改一两处比重跑一整轮划算**，所以两处都能改正文：

- **生成结果**（第 3 栏每篇的「人工修改」）：改完保存会重新跑一遍校验器，并在改写说明里记「人工修改：是」。模型直出的违规内容一律不落盘；人工改过、改完仍不合规的，要勾上「允许落盘未通过校验的人工修改」才落——改的人自己就是判断依据，但校验器的意见照样记进落盘回执与报告，不悄悄放行。
- **已经落盘的文件**（skill 库面板每行的「编辑」）：读回磁盘上的正文，改完写回。只允许改**已经存在**的 `.md`（新建仍走生成那条路，页面不该变成一个能往任意路径写文件的口子）；写回前先落到同目录的临时文件上过一遍 `validate_skills` 的校验，有 ERROR 就不动原文件，除非勾「有 ERROR 也照存」。

页面底部常驻「现有 skill 库」面板：按一级目录列出落盘目录里已有的 skill 与它们的 description，落盘目标已经在库里时，那一行会标「已存在，落盘会覆盖」——覆盖和新增的区别要在按下落盘之前就看得见，写完面板自动刷新。上传处的「skill 放在哪个一级目录」就是流水线的 `CATEGORY`：固定目录时全部落在 `排障步骤/<告警名>.md`，按工作簿文件名时一个表格一个目录（一次传多张表时能避开同名场景撞路径）。
...
这份测试真起服务、真发浏览器那种 multipart 请求传 xlsx、真过校验器、真写文件，只把 `requests.post` 换掉，守住这几件事：上传阶段一次模型都不调、生成后能拿到全文、三种下载都对、落盘写出文件且重复落盘不重复写、落盘后能在 skill 库面板里看到它、人工修改会被重新校验且改坏了默认落不了盘（明说之后才落，回执里记明未通过校验）、改已落盘文件写得回去而带 ERROR 的改动写不进去、违规回复判失败且一个文件都不写出去、坏输入（非 xlsx / 越界的场景号 / 不存在的任务号 / 越出输出目录的路径）落在 4xx。
