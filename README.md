...
**命令是逐词比的，参数位置当通配**。表里的参数不一定写成 `<>`：`display cpu-usage process process-id` 是裸参数名，`display cpu-usage process [ slot slot-id ]` 里的方括号是「可选参数」的语法记号。正文写成 `<process-id>`、`slot <slot-id>` 或整段省掉都算数，逐字比就只有原样照抄才能过，而照抄的裸参数名和方括号 agent 会当成关键字敲进设备——两条要求互相打架，模型三次重试全废。
