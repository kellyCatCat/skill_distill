...
**怎么读表**：`XLSX_PATH` 指向目录时处理其下**全部**工作簿，每个工作簿遍历**所有 sheet**（一个工作簿按协议分几个 sheet 是常态，只读第一个会静默漏掉）。

列**按表头认，不按列序**：各家的表列数不一样（有的没有「排障目标」，有的连「回显」「配置修复建议」「修复验证」都没有），写死列序会整表错位——把「步骤详细描述」当成「命令行」读，后面每一道校验都跟着错。只有「排障步骤编号」和「步骤详细描述」是必需的，缺了这两列的 sheet 当说明页跳过；`--check` 会打印每张表认出了哪些列、少了哪些列。

一个 sheet 里的多个场景靠**场景列**分块（「排障目标」，没有这一列就用「组网场景」）：该列的值**变了**才是下一个场景。合并单元格天然满足，而不合并、每行把排障目标重填一遍的表也一样能切——只看"这格非空"的话，那种表会被切成每行一个场景，一个八步的排障流程变成八篇一步的 skill。这一列整列为空时（有的表那格放的是截图）退回按**步骤编号**分块，编号回到 1 就是下一个场景。场景名依次退：排障目标 → 组网场景 → sheet 名。

ragIndex 那格写了多行带编号的用途（`1. 查看…` 换行 `2. 查看…`）时整格当用途、不取编号——那是给该步的两条命令各写了一句，把开头那个 1 当成 ragIndex 的话，另一步的 `1.` 就会和它凑成假的「一号两命令」。

**根因清单靠模型读懂表意，不靠正则**：每个场景改写前先花一次调用（`distill_root_causes`，temperature=0）抽出这张表写明的判定结论，清单写进 prompt 让「根因定位」和根因对照表两处逐字一致，也用来校验对照表有没有漏根因。原先是正则捞 `提示/返回"xxx"` 的字面写法，可各家表述根本不统一——"认为是报文攻击导致CPU冲高"、"初步定位原因是路由协议震荡"都是结论，正则一条也捞不到，再加几种句式只是把下一张表的漏判往后推。抽不出来时清单为空、照常改写，只是不校验根因覆盖，报告的「根因清单」一行会写明。不调模型的路径（`--check`、`--validate`、单测）拿到的都是空清单。
...
最要紧的一条是**只能用步骤表里出现过的 CLI，一条都不许自己生成**。命令来源是**除「回显」外的各列**（「命令行」「配置修复建议」「修复验证」「步骤详细描述」「修复建议影响性」——最后一列也算，因为实际的表里修复 CLI 就有填在那一格的，排除它会把照着它写的命令判成编造的），**「回显」列不算**——回显是某台设备当时的输出（里面有 `bgp 100`、`segment-list 1`），把它当命令来源等于给"照着回显编一段配置"开后门，而那正是要拦的东西。表里只给了"减少 policy 数量"这种修复方向时，就照实写这句方向，不要补全成可执行的配置序列。

**校验没过时是让模型「改」而不是「重写」**：重试时把上一次输出的 skill 全文连同失败原因一并发回去，只让它动出错那一处、其余原样抄回。只给原因不给原文的话模型看不到自己写了什么，同一处错误能连犯三次（实测把表里的具体值换成参数、给命令加过滤条件、漏写一条命令，都是三次全废）。

**校验器会拦下的东西**（不合规的不落盘，只记进报告，并把原因回传给模型让它改）：命令没用反引号、参数不是 `<>` 形式、同一参数多种写法、残留表内编号、编造表里没有的命令、修复 CLI 里抄了回显的具体值、正文用了没申报的参数、前置检查里有跳转、根因对照表漏了步骤表写明的根因、跳转指向不存在的步骤。报错会带上**步骤表位置**（步骤号、sheet、行号）和正文里最接近的那段命令；编造命令时会连表里可用的命令一起列出来，参数类报错会指出是哪条命令带着这个参数；参数名被改过时（表里 `car-index`、正文写成 `<car-id>`）会指出表里的写法——说成"这条 CLI 是自己编的、删掉它"会让模型把对的命令删了。

**命令是逐词比的，参数位置当通配**。「命令行」格里一行一条命令，行首的 `1.` `2、` `-` 这类列表编号会被剥掉——编号是填表人排版用的，留着就会要求正文里出现一条 `1.` 开头的命令。表里的参数不一定写成 `<>`：`display cpu-usage process process-id` 是裸参数名，`display cpu-usage process [ slot slot-id ]` 里的方括号是「可选参数」的语法记号。正文写成 `<process-id>`、`slot <slot-id>` 或整段省掉都算数，逐字比就只有原样照抄才能过，而照抄的裸参数名和方括号 agent 会当成关键字敲进设备——两条要求互相打架，模型三次重试全废。
...
三张基准表、三份格式基准，各守一种表的长相：`排障步骤表.xlsx` + `sample_skill.md` 是 11 列的完整形态；`CPU利用率超限步骤表.xlsx` + `sample_skill_cpu.md` 只有 6 列（没有排障目标/回显/修复建议/影响性/修复验证），参数是裸词或 `[ ]` 圈着的，还有一条命令只出现在「步骤详细描述」里；`CPU利用率超限定位步骤表.xlsx` + `sample_skill_cpu_alarm.md` 列全，但排障目标每行重填而非合并、表尾多一列自定义列、ragIndex 那格是带编号的两条用途、修复 CLI 填在了「影响性」那格里。xlsx 是二进制、进 git 看不出 diff，所以表的内容以 `excel_cases/build_*_sheet.py` 为准，改表改脚本再重跑。

`mock_run_excel_pipeline.py` 只替换 `requests.post` 一层，retry、SSE 解析、extractor 校验、多进程、报告全是真的在走，覆盖五种情形（抽根因那次调用的假回复按 prompt 分派，一律回同一份的话这条链路坏了也没人发现）：合规回复 + 普通 JSON、合规回复 + SSE 流（要与 JSON 路径逐字一致、中文不乱码）、违规回复（必须被拦下且一个文件都不许写出去）、先违规后合规（校验失败要把原因回传给模型，第二次通过）、列不一样的第二张表整条跑通。
...
| --- | --- | --- |
| `cases/` | 新增故障案例的原始数据 | ✅ |
| `excel_cases/` | 排障步骤表 xlsx（由 `build_*_sheet.py` 生成），以及格式基准 `sample_skill.md`、`sample_skill_cpu.md`（测试要读） | ✅ |
| `evals/` | 排障评测结果 | ❌ |
...
- `excel_skill_distill_pipeline.py` 默认处理 `excel_cases/` 下的**每一个** xlsx，其中包括仓库自带的三张样例表 `排障步骤表.xlsx`、`CPU利用率超限步骤表.xlsx`、`CPU利用率超限定位步骤表.xlsx`（它们同时是测试的输入，别删）。只想跑自己那张表时，把 `XLSX_DIR` 指到具体文件。
